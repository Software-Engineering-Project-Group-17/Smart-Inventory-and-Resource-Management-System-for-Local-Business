...
const parsePeriodDays = (period = '30d') => {
  switch (period) {
    ...
    case '30d': return 30;
    case '90d': return 90;
    default:    return clampInt(period, 30, 1, 365); // allow numeric days too
  }
};
 ...
 * Query: branchId?, period? (7d|30d|90d|<number_of_days>)
 */
...
const getQuickInsights = async (req, res) => {
  try {
    ...
    const params = branchId ? [start, branchId, prevStart] : [start, prevStart];
    const { rows } = await query(sql, params);
    const r = rows[0] || {};
    ...
    const insights = [
      ...
      r.top_category ? {
        type: 'info',
        text: `Top category: ${r.top_category}`
      } : null,
      (Number.isFinite(parseInt(r.low_stock_items, 10)) && parseInt(r.low_stock_items, 10) > 0) ? {
        type: 'alert',
        text: `${parseInt(r.low_stock_items, 10)} item(s) are at or below low-stock threshold`
      } : null
    ].filter(Boolean);
  ...
  try {
    ...
    const sql = `
      ...
      low_stock AS (
        ...
        FROM inventory_item
        ${branchId ? 'WHERE branch_id = $2' : ''}
          ${branchId ? 'AND' : 'WHERE'} quantity <= low_stock_threshold
      )
      SELECT 
    ...
    if (parseFloat(r.prev_rev || 0) > 0) {
      ...
      if (diffPct <= -10) {
        alerts.push({
          type: 'revenue-drop',
          severity: 'high',
          message: `Revenue down ${Math.abs(diffPct).toFixed(1)}% vs prior window`
        });
      } else if (diffPct <= -5) {
        alerts.push({
          type: 'revenue-drop',
          severity: 'medium',
          message: `Revenue dipped ${Math.abs(diffPct).toFixed(1)}% vs prior window`
        });
      }
    }
    ...
    if (parseFloat(r.prev_rev || 0) > 0) {
      ...
      if (diffPct >= 15) {
        alerts.push({
          type: 'revenue-spike',
          severity: 'info',
          message: `Revenue up ${diffPct.toFixed(1)}% vs prior window`
        });
      }
    }
