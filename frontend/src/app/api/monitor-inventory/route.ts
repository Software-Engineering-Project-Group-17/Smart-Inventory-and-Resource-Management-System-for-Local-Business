...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`Found ${lowStockItems.length} items at or below low stock threshold`);
    ...
    for (const item of lowStockItems) {
      try {
        ...
        if (existingNotification.length === 0) {
          ...
          console.log(`Created low stock notification for ${item.inventory_name} (ID: ${item.inventory_id})`);
        } else {
          console.log(`Skipping ${item.inventory_name} - recent notification exists (${existingNotification[0].created_at})`);
        }
      } catch (itemError) {
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      ...
      stats: {
        ...stats[0],
        recent_low_stock_notifications: recentNotifications[0].recent_low_stock_notifications
      },
