...
function SalesPage() {
  ...
  const [showMobileScannerUrl, setShowMobileScannerUrl] =
    useState<boolean>(false);
  ...
  const generateQRCode = async () => {
    try {
      const baseUrl = process.env.NEXT_PUBLIC_PROTOCOL === 'https' 
        ? `https://${process.env.NEXT_PUBLIC_SERVER_IP}`
        : `https://${process.env.NEXT_PUBLIC_SERVER_IP}:${process.env.NEXT_PUBLIC_SERVER_PORT}`;
      const scannerUrl = `${baseUrl}/scanner?user=${encodeURIComponent(userEmail)}`;
  ...
  useEffect(() => {
    if (lastScanEvent) {
      ...
      console.log(
        "🔍 Barcode received via WebSocket:",
        barcode,
        "Scan ID:",
        lastScanEvent.scanId
      );
  ...
  const searchInventory = async (term: string, type: "name" | "barcode") => {
    ...
    try {
      const response = await fetch(
        `/api/sales?search=${encodeURIComponent(
          term
        )}&type=${type}&userEmail=${encodeURIComponent(userEmail)}`
      );
      ...
      if (data.success) {
        ...
        if (
          ...
          lastScanEvent &&
          term === lastScanEvent.barcode
        ) {
          console.log(
            "🔍 Auto-selecting first barcode result:",
            data.inventory[0]
          );
  ...
  const handleCompleteSale = async () => {
    ...
    try {
      const saleData = {
        ...
        customerInfo:
          customerInfo.name && customerInfo.phone ? customerInfo : null,
        paymentAmount: payment,
        loyaltyPointsUsed:
          customerInfo.isRegistered && loyaltyPoints > 0 ? loyaltyPoints : 0,
  ...
  const loyaltyPointsUsed = customerInfo.isRegistered
    ? Math.min(loyaltyPoints, total)
    : 0;
  const finalTotal = Math.max(0, total - loyaltyPointsUsed);
  const preliminaryBalance = payment - finalTotal;
  const loyaltyPointsToAdd =
    customerInfo.isRegistered && preliminaryBalance > 0
      ? Math.min(addToLoyaltyPoints, preliminaryBalance)
      : 0;
        ...
        <div className="bg-white rounded-lg shadow p-4 sm:p-6 mb-4 sm:mb-6 border border-gray-100">
          <div className="flex flex-col lg:flex-row justify-between items-center">
            <div className="flex items-center space-x-3 mb-4 lg:mb-0">
              <div
                className="w-12 h-12 sm:w-14 sm:h-14 rounded-lg shadow flex items-center justify-center"
              ...
              <div className="flex items-center space-x-2">
                ...
                >
                  <span className="font-medium">
                    {isClient ? currentDate : new Date().toLocaleDateString()}
                  </span>
                </ClientOnly>
              </div>
            ...
            <div className="bg-white rounded-lg shadow p-4 sm:p-6 mb-4 sm:mb-6 border border-gray-100">
              ...
              <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-3 mb-4 sm:mb-6">
                <button
                  ...
                  style={
                    searchType === "name" ? { backgroundColor: "#3674B5" } : {}
                  }
                >
                  Search by Name
                </button>
                ...
                <button
                  onClick={async () => {
                    try {
                      const response = await fetch(
                        `/api/sales?userEmail=${encodeURIComponent(userEmail)}`
                      );
                      const data = await response.json();
                ...
                <div className="mb-6 p-4 bg-gradient-to-r from-green-50 to-blue-50 border border-green-200 rounded-lg">
                      ...
                      <div className="bg-white p-3 rounded-lg border">
                        ...
                        <code className="text-xs font-mono text-blue-600 break-all">
                          {process.env.NEXT_PUBLIC_PROTOCOL === 'https' 
                            ? `https://${process.env.NEXT_PUBLIC_SERVER_IP}/scanner?user={encodeURIComponent(userEmail)}`
                        ...
                        <button
                          onClick={() => {
                            const baseUrl = process.env.NEXT_PUBLIC_PROTOCOL === 'https' 
                              ? `https://${process.env.NEXT_PUBLIC_SERVER_IP}`
                              : `https://${process.env.NEXT_PUBLIC_SERVER_IP}:${process.env.NEXT_PUBLIC_SERVER_PORT}`;
                            const url = `${baseUrl}/scanner?user=${encodeURIComponent(userEmail)}`;
                  ...
                  <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                    <div className="flex items-start space-x-2 text-xs text-blue-700">
                      ...
                      <div>
                        <strong>Tip:</strong> Most phones can scan QR codes
                        directly with their camera app. Just point your camera
              ...
              {selectedItem && (
                ...
                >
                      ...
                      <div className="flex space-x-2">
                        <select
                          ...
                          onChange={(e) =>
                            setDiscount({
                              ...discount,
                              type: e.target.value as "value" | "percentage",
                            })
                          }
                  ...
                  <button
                    ...
                    style={{
                      backgroundColor:
                        quantity > selectedItem.stock ? "#ccc" : "#3674B5",
                    }}
                    onKeyDown={(e) => {
                      if (e.key === "Enter") {
                        e.preventDefault();
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b-2 border-gray-200">
                        <th className="text-left py-4 px-2 font-bold text-gray-700">
                          Item
                        </th>
                        <th className="text-center py-4 px-2 font-bold text-gray-700 hidden sm:table-cell">
                          Price
                        </th>
                        <th className="text-center py-4 px-2 font-bold text-gray-700">
                          Qty
                        </th>
                        <th className="text-center py-4 px-2 font-bold text-gray-700 hidden sm:table-cell">
                          Discount
                        </th>
                        <th className="text-right py-4 px-2 font-bold text-gray-700">
                          Total
                        </th>
                        <th className="text-center py-4 px-2 font-bold text-gray-700">
                          Action
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      Enter 10+ digits to automatically search for existing
                      customer
                    </p>
                  </div>
                  ...
                  {!customerInfo.phone && !showCustomerRegistration && (
                    <div className="space-y-3">
                      ...
                      <p className="text-xs text-gray-500">
                        For registered customers with loyalty points, enter
                        phone number above
                      </p>
                    </div>
                  )}
            ...
            <div className="bg-white rounded-lg shadow p-4 sm:p-6 border border-gray-100">
              ...
              <div
                className={`space-y-4 mb-6 ${
                  customerInfo.isRegistered
                    ? "sm:flex sm:flex-row sm:space-y-0 sm:space-x-4 justify-between"
                    : ""
                }`}
              >
              ...
              {payment > 0 && (
                <div
                  className="p-2 rounded-lg mb-6 border-2"
                  style={{
                    backgroundColor: finalBalance >= 0 ? "#E7F5E7" : "#FFE7E7",
                    borderColor: finalBalance >= 0 ? "#10B981" : "#EF4444",
                  }}
                >
                  <div className="flex justify-between text-base font-bold">
                    <span>
                      {finalBalance >= 0 ? "Change:" : "Balance Due:"}
                    </span>
                    <span
                      className={
                  ...
                  {finalBalance < 0 && (
                    <p className="text-sm text-red-600 mt-2 font-medium">
                      Need ${Math.abs(finalBalance).toFixed(2)} more to complete
                      transaction
                    </p>
                  )}
                  {loyaltyPointsToAdd > 0 && (
                    <p className="text-sm text-blue-600 mt-2 font-medium">
                      ${loyaltyPointsToAdd.toFixed(2)} will be added to loyalty
                      points
                    </p>
                  )}
                </div>
              ...
              <button
                ...
                style={{
                  backgroundColor:
                    items.length === 0 || finalBalance < 0 || loading
                      ? "#ccc"
                      : "#3674B5",
                }}
              >
              ...
              <div className="mt-4 grid grid-cols-2 gap-2">
                ...
                <button
                  ...
                  style={{
                    backgroundColor: items.length === 0 ? "#ccc" : "#FADA7A",
                    color: items.length === 0 ? "#666" : "#3674B5",
                  }}
                >
                  Exact Amount
                </button>
