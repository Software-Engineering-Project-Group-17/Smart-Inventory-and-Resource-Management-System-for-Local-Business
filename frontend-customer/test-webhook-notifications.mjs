...
async function testWebhookNotificationLogic() {
  ...
  try {
    ...
    const result = await NotificationService.checkAndCreateLowStockNotification(11, 3);
    ...
    if (result) {
      ...
      console.log('ℹ️ No notification created (stock above threshold or recent notification exists)');
    }
    
    // Test with Tape Measure (ID: 14, Branch: 3) - also known to be low stock
    console.log('\nTesting with Tape Measure (ID: 14, Branch: 3)...');
    
    const result2 = await NotificationService.checkAndCreateLowStockNotification(14, 3);
